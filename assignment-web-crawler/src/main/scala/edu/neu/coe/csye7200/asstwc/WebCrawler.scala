...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for(n <- ns \\ "a" map (_ \@ "href")) yield createURL(Option(u), n) // TO BE IMPLEMENTED
    ...
    // 9 points.
    for (xs <- getURLContent(u); x <- MonadOps.asFuture(getLinks(xs))) yield x // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    val x1: Seq[Future[Seq[URL]]] = for (u <- us2) yield wget(u)
    val x2: Seq[Future[Either[Throwable, Seq[URL]]]] = for (x <- x1) yield MonadOps.sequence(x)
    Future.sequence(x2)
  }
